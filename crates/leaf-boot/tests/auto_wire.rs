...
use leaf_core::{
    ...
    ErasedArgs, ErasedBean, ErasedRet, Interceptor, LeafError, MethodEntry, MethodJoinPointSpec,
    MethodKey, MethodTable, Next, Origin, Provider, ProviderSeed, Published, Ref, ResolveCtx, Role,
    RunState, Runner, ScopeDef,
};
...
/// A `Runner` bean registered via the auto-config ladder + auto-collected from the
/// live Context (the `RunnerPairing` upcast thunk re-wraps the erased bean as
/// `Arc<dyn Runner>`).
#[derive(Debug)]
struct MigrateRunner;
impl leaf_core::Bean for MigrateRunner {}
...
struct MigrateRunnerProvider {
    descriptor: Descriptor,
}
impl Provider for MigrateRunnerProvider {
    fn descriptor(&self) -> &Descriptor {
        &self.descriptor
    }
    fn provide<'a>(&'a self, _cx: &'a ResolveCtx<'a>) -> BoxFuture<'a, Result<Published, LeafError>> {
        Box::pin(async move { Ok(Published::shared_value(MigrateRunner)) })
    }
}

/// The `dyn Runner` upcast view (what `#[runner]` emits in `provides[]`).
fn runner_provides() -> &'static [leaf_core::TypeRow] {
    static ROWS: &[leaf_core::TypeRow] = &[leaf_core::TypeRow {
        view: TypeId::of::<dyn Runner>(),
        upcast: |b: ErasedBean| -> ErasedBean { b },
    }];
    ROWS
}

fn migrate_runner_descriptor() -> Descriptor {
    Descriptor {
        contract: ContractId::of(concat!(module_path!(), "::MigrateRunner")),
        self_type: TypeId::of::<MigrateRunner>(),
        provides: runner_provides(),
        declared_name: Some("migrateRunner"),
        aliases: &[],
        scope: ScopeDef::SINGLETON,
        role: Role::Application,
        meta: &AnnotationMetadata::EMPTY,
...
const fn migrate_runner_seed() -> ProviderSeed {
    || {
        Arc::new(MigrateRunnerProvider {
            descriptor: Descriptor {
                contract: ContractId::of(concat!(module_path!(), "::MigrateRunner")),
                self_type: TypeId::of::<MigrateRunner>(),
                provides: runner_provides(),
...
/// The macro-emitted runner upcast thunk: downcast the erased bean to the concrete
/// runner type, re-wrap as `Arc<dyn Runner>` (the per-runner thunk `#[runner]` emits).
fn migrate_runner_upcast(b: ErasedBean) -> Option<Arc<dyn Runner>> {
    b.downcast::<MigrateRunner>().ok().map(|a| a as Arc<dyn Runner>)
}
...
// The macro-emitted per-bean join-point spec (the `__leaf_joinpoints_<Ident>` const).
static ORDER_SERVICE_METHODS: &[MethodJoinPointSpec] = &[MethodJoinPointSpec {
    method: MethodKey::of("OrderService::place_order"),
    arg_types: &[],
    ret_type: TypeId::of::<i64>(),
}];
static ORDER_SERVICE_SPEC: leaf_core::BeanJoinPointsSpec = leaf_core::BeanJoinPointsSpec {
    bean_type: TypeId::of::<OrderService>(),
    markers: &AnnotationMetadata::EMPTY,
    methods: ORDER_SERVICE_METHODS,
};

// The macro-emitted per-bean MethodTable (the downcast thunks the transparent proxy
// drives — `__leaf_methods_<Ident>`).
fn place_order_invoke(
    bean: &ErasedBean,
    args: ErasedArgs,
    _cx: &ResolveCtx<'_>,
) -> BoxFuture<'static, Result<ErasedRet, AdviceError>> {
    let svc = Arc::clone(bean)
        .downcast::<OrderService>()
        .expect("the advised bean downcasts to OrderService");
    Box::pin(async move {
        let amount = args
            .unpack::<i64>()
            .map_err(|_| AdviceError::DowncastMismatch { method: MethodKey::of("OrderService::place_order") })?;
        Ok(ErasedRet::pack(svc.place_order(amount)))
    })
}
static ORDER_SERVICE_TABLE: MethodTable = MethodTable(&[MethodEntry {
    key: MethodKey::of("OrderService::place_order"),
    invoke: place_order_invoke,
}]);
...
async fn a_real_annotated_app_auto_wires_proxy_runner_and_config() {
    ...
    // The seed JOIN (the two #[component]s).
    let seeds = vec![
        SeedPairing::new(repo_contract, __leaf_seed_Repository),
        SeedPairing::new(service_contract, __leaf_seed_OrderService),
    ];

    // The config-properties + runner beans register via the auto-config ladder.
    let autoconfig = vec![
        leaf_boot::AutoConfigCandidate::new(app_props_descriptor(), app_props_seed(), None),
        leaf_boot::AutoConfigCandidate::new(
            migrate_runner_descriptor(),
            migrate_runner_seed(),
            None,
        ),
    ];
    ...
    let running = Application::new()
        ...
        .with_join_points(vec![JoinPointPairing::new(service_contract, &ORDER_SERVICE_SPEC)])
        .with_method_tables(vec![MethodTablePairing::new(service_contract, &ORDER_SERVICE_TABLE)])
        // The runner bean is AUTO-COLLECTED from the live Context (no with_runner).
        .with_runner_beans(vec![RunnerPairing::new(runner_contract, migrate_runner_upcast)])
