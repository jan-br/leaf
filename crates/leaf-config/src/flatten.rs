...
pub enum Node {
    /// A scalar already rendered to its canonical string form.
    Scalar(String),
...
/// Flatten `root` into canonical `(key, value)` pairs, stamping every value with
/// `origin`. The top-level `prefix` is empty (so a top-level map key is the bare
/// key); nested keys join with `.` and list positions with `[i]`.
#[must_use]
pub fn flatten(root: &Node, origin: Origin) -> Vec<FlatEntry> {
    let mut out = Vec::new();
    walk("", root, origin, &mut out);
    out
}

fn walk(prefix: &str, node: &Node, origin: Origin, out: &mut Vec<FlatEntry>) {
    match node {
        ...
        Node::Scalar(s) => {
            ...
            if !prefix.is_empty() {
                out.push((
                    prefix.to_string(),
                    PropertyValue::with_origin(s.clone(), origin),
                ));
            }
        }
        Node::Map(entries) => {
...
mod tests {
    ...
    fn flattens_nested_object_with_dotted_keys() {
        let node = Node::Map(vec![(
            "server".to_string(),
            Node::Map(vec![
                ("port".to_string(), Node::Scalar("8080".to_string())),
                ("host".to_string(), Node::Scalar("localhost".to_string())),
            ]),
        )]);
        assert_eq!(
            entries(&node),
    ...
    fn flattens_arrays_with_bracket_index_segments() {
        let node = Node::Map(vec![(
            "hosts".to_string(),
            Node::Seq(vec![
                Node::Scalar("a".to_string()),
                Node::Scalar("b".to_string()),
            ]),
        )]);
        assert_eq!(
            entries(&node),
    ...
    fn null_is_absent_and_empty_containers_emit_nothing() {
        let node = Node::Map(vec![
            ...
            ("d".to_string(), Node::Scalar("kept".to_string())),
        ]);
