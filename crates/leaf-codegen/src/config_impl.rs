...
mod tests {
    ...
    // ── the advisable impl no longer lowers a `#[inject]` constructor (Task 3) ─────
    // The `#[inject]`-marker / `#[advisable]`-constructor / runtime-merge path is REMOVED:
    // a state-holding or collaborator-injecting bean now wears `#[repository(constructor =
    // <path>)]` on the STRUCT (the referenced-constructor stereotype path), so the impl
    // macro emits ONLY proxy metadata — never a construction seed/plan.

    #[test]
    fn an_advisable_impl_with_a_plain_new_emits_no_ctor_wiring() {
        // An `#[advisable]` impl with a plain (un-marked) `fn new` emits only the proxy
        // metadata (join points + method table) — the stereotype macro owns construction.
        // No constructor SEED/PLAN rows here.
        let item = impl_item(
            "impl OrderService {
                fn new(repo: Ref<Repository>) -> Self { todo!() }
                fn place_order(&self, amount: i64) -> i64 { todo!() }
        ...
        assert!(
            !s.contains("#[::leaf_core::linkme::distributed_slice(::leaf_core::SEED_PAIRINGS)]"),
            "an advisable impl emits no construction SEED_PAIRINGS row: {s}"
        );
    }

    #[test]
    fn an_advisable_impl_with_an_inject_marked_new_emits_no_constructor_wiring() {
        // POST-REMOVAL (Task 3): `#[inject]`-constructor lowering is gone — the
        // referenced-constructor `constructor = <path>` stereotype path supersedes it.
        // An `#[advisable]` impl whose `fn new(..)` is `#[inject]`-marked emits ONLY the
        // proxy metadata (join points + method table); the `#[inject]` marker is no longer
        // read, so NO constructor seed/plan row, NO `from_constructor`, NO `construct_with`
        // / `ctor_deps`, and NO `emit_wiring_only` artifact rides the impl's tokens.
        let item = impl_item(
            "impl OrderService {
                #[inject]
        ...
        let s = flat(&emit_advisable_impl(&item).expect("emits"));
        assert!(
            s.contains("pubstatic__leaf_methods_OrderService"),
            "the proxy metadata still emits: {s}"
        );
        assert!(
            !s.contains("#[::leaf_core::linkme::distributed_slice(::leaf_core::SEED_PAIRINGS)]"),
            "the #[inject] constructor lowering is removed => no SEED_PAIRINGS row: {s}"
        );
        assert!(
            !s.contains("from_constructor"),
            "no `from_constructor` precedence row is emitted: {s}"
        );
        assert!(
            !s.contains("construct_with") && !s.contains("ctor_deps"),
            "no referenced-constructor driver is emitted by the advisable impl path: {s}"
        );
    }
}
