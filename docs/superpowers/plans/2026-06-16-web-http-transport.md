...
- [x] **Step 1 (test):** an umbrella-only integration test (under `crates/leaf` or a fixture) with `use leaf::prelude::*;` defines a `#[rest_controller]` with a `#[get]` and boots — proving the macro paths resolve through the facade aliases (mirrors the existing `::leaf_cache::`/`::leaf_tx::` alias pattern). DONE — `crates/leaf/tests/web_umbrella.rs` (a `#[rest_controller]` with `#[get]`+`#[post]`, a `#[component]` `WebFilter`, a `#[control_advice]`; boots on a dedicated thread + probes the bound socket umbrella-only).
- [x] **Step 2:** FAIL (unresolved macro/type paths) — confirmed: 18 errors (`rest_controller`/`get`/`post`/`control_advice`/`exception_handler` macros + `Path`/`Json`/`Request`/`Response`/`Next`/`WebFilter` types unresolved through the prelude).
- [x] **Step 3:** wire the bundle + feature + prelude + facade re-exports. DONE: `leaf-starter-web` now bundles `leaf-web`+`leaf-web-hyper`+`leaf-serde` (+ the runtime/concern peers); the umbrella root re-exports the macro-referenced `::leaf_web::` surface (incl. `leaf_web::http` for the verb token) gated on `web`; the prelude re-exports the web macros + extractor/extension types gated on `web`; `#[leaf::main]` auto-emits `extern crate leaf as leaf_web;`; the `web` participating set / `ExpectedManifest` now mirrors the real bundle. The web macros' external `http` token was retargeted `::http::` → `::leaf_web::http::` so it rides the one `leaf_web` alias (no `http` direct dep for an umbrella-only app).
- [x] **Step 4:** PASS — `web_umbrella` boots+serves umbrella-only; the base-path `proof_app` boot test is `not(feature="web")`-gated (the bundle's blocking `WebServerRunner` makes `run()` not return under `web`).
- [x] **Step 5: gate + commit.** DONE — `cargo test --workspace` + `-p leaf --features web` (0 failed), `clippy --workspace --all-targets -D warnings` + the web-feature variant (exit 0), `cargo doc` broken-intra-doc-links gate + the web-feature variant (exit 0).

> **Note for Task 14:** `Json<T>: FromRequest` is NOT implemented anywhere — the Task-9 controller codegen uniformly emits `<Param as FromRequest>::from_request`, but `leaf-web`'s `Json<T>` has no `FromRequest` impl (by design it rides the injected converter), and no such impl exists in `leaf-serde` either. A `Json<_>` handler parameter is therefore currently uncompilable. Task 14's `POST /orders` body needs this gap closed (structurally dispatch `Json<_>`/`Query<T>`/`State<_>`/`Header<_>` in the codegen, or add the converter-backed extraction) — it surfaced here and was left to Task 14 to keep Task 13 scoped to the facade wiring.
